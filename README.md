# Hi there, I'm Ankit Jangid! 👋

### 🎓 MCA Student | Aspiring Software Developer | Tech Enthusiast

I am a Computer Applications student based in **Jaipur, Rajasthan**, with a strong academic background and a growing passion for software development and problem-solving.

Currently, I am pursuing my **Master of Computer Applications (MCA)** from **Rajasthan Technical University, Kota**, where I am strengthening my skills in programming, data structures, and modern web technologies. I enjoy building practical projects that connect theoretical knowledge with real-world applications.

---

### 🛠️ Tech Stack & Skills

- **Programming Languages:** C, C++, Python, Java, Data Structures & Algorithms (DSA), SQL  
- **Web Technologies:** HTML, CSS, JavaScript  
- **Databases:** MySQL  
- **Tools & IDEs:** Visual Studio Code, Eclipse  
- **Office Tools:** MS Word, MS Excel, MS PowerPoint  
- **Soft Skills:** Problem Solving, Analytical Thinking, Communication, Adaptability  

---

### 🔭 Projects

#### 🛒 ProShop – Modern E-Commerce Website
- Developed a fully responsive e-commerce website using **HTML, CSS, and JavaScript**
- Implemented **user authentication (Login & Signup)** using **Google Firebase**
- Focused on clean UI design and user experience
- 🌐 **Live Demo:** https://proshop-modern-e-commerce.netlify.app/

#### 🎓 Student Information Management System
- Python-based console application to manage student records
- Implemented **Object-Oriented Programming (OOP)** concepts
- Designed for academic learning and practical understanding of software structure

---

### 🎯 Career Objective

I aim to become a skilled **Software Developer**, continuously learning new technologies and building impactful applications. I am particularly interested in **backend development, problem-solving, and full-stack projects**.

---

### 📜 Certifications

- **Python Fundamentals Certification** – Infosys Springboard (March 2025)  
- **Front-End Web Development** – Reliance Foundation Skilling Academy via Skill India Digital Hub (November 2025)  
- **2-Day Workshop on Data Science & Analytics** – GRRAS Solutions Pvt. Ltd. (March 2025)  

---

### 📫 Connect with Me

- **Email:** ankitjangid2025@gmail.com  
- **Location:** Jaipur, Rajasthan, India  
- **Portfolio / Project:** https://proshop-modern-e-commerce.netlify.app/
